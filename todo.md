...
- [x] Добавить статус «Мало на складе» для товаров с остатком менее 10 единиц
- [x] Синхронизировать обновления с сервером 188.225.73.61
- [x] Настроить автоматическую синхронизацию с сервером через GitHub Actions CI/CD
- [x] Сделать логотип в футере читаемым (светлый на тёмном фоне)
- [x] Убрать фразу "Принимаем к оплате:" из футера
- [x] Добавить ссылку на главную страницу для логотипа в футере
- [x] Добавить ссылку на главную страницу для логотипа в хедере
- [x] Проверить и добавить упоминание СБП в юридических документах

## Email-уведомления о заказах
- [x] Проверить подключение к почтовому серверу mail.gzakroma.ru (SMTP порт 25, AUTH LOGIN)
- [ ] Реализовать отправку email покупателю (noreply@gzakroma.ru) с деталями заказа
- [ ] Реализовать отправку email менеджеру (sales@gzakroma.ru) с деталями заказа
- [ ] Создать HTML-шаблоны писем
- [ ] Протестировать отправку с production сервера
